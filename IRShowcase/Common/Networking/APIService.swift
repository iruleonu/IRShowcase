...
protocol APIFetchPostsProtocol {
    func fetchPosts() -> SignalProducer<[Post], APIServiceError>
}
...
extension APIServiceImpl: APIFetchPostsProtocol {
    func fetchPosts() -> SignalProducer<[Post], APIServiceError> {
        let properties = Resource.posts.requestProperties
        let urlRequest = URLRequest(url: apiBaseUrl.appendingPathComponent(properties.path))
        
        let parseData: ((Data, URLResponse)) -> SignalProducer<[Post], APIServiceError> = { tuple in
            return SignalProducer { observer, _ in
                do {
                    let results = try JSONDecoder().decode([Post].self, from: tuple.0)
