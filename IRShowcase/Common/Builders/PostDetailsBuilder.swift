...
protocol PostDetailsChildBuilders {
    // Empty
}

struct PostDetailsBuilder: PostDetailsChildBuilders {
    // swiftlint:disable function_parameter_count
    func make(navigation: UINavigationController?, postId: Int, network: DataProviderNetworkProtocol, persistence: DataProviderPersistenceProtocol, connectivity: ConnectivityService, action: @escaping (PostListAction) -> Void) -> UIViewController {
        ...
        let viewModel = PostDetailsViewModelImpl(routing: coordinator, postId: postId, userDataProvider: userDataProvider, postDataProvider: postDataProvider, commentsDataProvider: commentsDataProvider, connectivity: connectivity)
