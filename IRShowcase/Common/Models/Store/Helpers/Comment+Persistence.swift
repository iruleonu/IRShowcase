...
extension Comment: CodableToPersistence {
...
extension Comment: CoreDataToCodable {
    static func mapToModel<T>(_ object: NSManagedObject) -> T? where T: Decodable, T: Encodable {
        ...
        do {
            ...
            let modelFromData = try JSONDecoder().decode(Comment.self, from: data)
            model = modelFromData
        } catch { }
